...
class PageModel extends LifeCycle<Params, Query> {
  constructor(props: LifeCycleProps<Params, Query>) {
    super(props);
    console.log('numIslands', this.getKthFromEnd2(list3, 2));
  }
  ...
  permute = (nums: Array<number>) => {
    ...
    const dfs = (path: Array<number>) => {
      ...
      for (const num of nums) {
        ...
        // 每一次递归结束，得到完整的一种结果后要（ 回溯 ）将刚刚选的数pop出来
        path.pop();
        // 我们不是找到一个排列就完事，要找出所有满足条件的排列。
        // 递归结束时，结束的是当前的递归分支，还要去别的分支继续搜。
        // 所以，要撤销当前的选择，回到选择前的状态，根据used[num]去选下一个选项，即切入下一个分支。
  ...
  numIslands = (grid: Array<Array<string>>) => {
    ...
    const determinScope = (grid: Array<Array<string>>, i: number, j: number) => {
      if (
  ...
  /**
   * @description: https://leetcode-cn.com/problems/lian-biao-zhong-dao-shu-di-kge-jie-dian-lcof/solution/lian-biao-zhong-dao-shu-di-kge-jie-dian-1pz9l/
   * @param {any} head
   * @param {number} k
   * @return {*}
   * 首先求出链表的长度 n，然后顺序遍历到链表的第 n - k 个节点返回即可
   */
  getKthFromEnd1 = (head: any, k: number) => {
    let n = 0;
    let node = head;
    // 先求出链表的长度
    while (node) {
   ...
   * 此时指针 fast 与 slow 二者之间刚好间隔 k 个节点。此时两个指针同步向后走，
   * 当第一个指针 fast 走到链表的尾部空节点时，则此时 slow 指针刚好指向链表的倒数第k个节点
   */
  getKthFromEnd2 = (head: any, k: number) => {
    let fast = head;
    let slow = head;
    while (fast && k > 0) {
